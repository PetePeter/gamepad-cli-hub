...
function buildHeader(dirPath: string): HTMLElement {
  ...
  clearDoneBtn.className = 'plan-header__btn plan-header__btn--secondary';
  clearDoneBtn.title = 'Clear all completed plan items in this directory';
...
async function handleApplyFromCanvas(item: PlanItem): Promise<void> {
  try {
    ...
    if (result?.success && result.path) {
      ...
      await deliverBulkText(targetSessionId, `work for you to do is here: ${filePath}\n`);
...
async function handleClearDone(): Promise<void> {
  try {
    const items = await window.gamepadCli.planList(currentDir);
    ...
    clearDonePlans.dirName = parts[parts.length - 1] || currentDir;
    clearDonePlans.visible = true;
    setClearDonePlansCallback(async () => {
      await window.gamepadCli.planClearCompleted(currentDir);
      await refreshCanvas();
    });
  } catch (err) {
