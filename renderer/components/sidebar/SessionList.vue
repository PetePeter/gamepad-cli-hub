...
function onCommitRename([sessionId, newName]: [string, string]): void {
  emit('commitRename', sessionId, newName);
}

function onRequestClose([sessionId, displayName]: [string, string]): void {
  emit('requestClose', sessionId, displayName);
}

function onSessionStateChange([sessionId, newState]: [string, string]): void {
