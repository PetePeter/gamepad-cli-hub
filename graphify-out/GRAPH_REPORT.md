...
## Corpus Check
- 364 files · ~3,121,460 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3084 nodes · 6055 edges · 79 communities detected
...
Cohesion: 0.01
Nodes (533): absoluteStoragePath(), actionToPtyData(), add(), addDependency(), addPlanAttachment(), addSession(), allocateHumanId(), answerCallback() (+525 more)

### Community 1 - "Community 1"
Cohesion: 0.01
Nodes (168): createSortControl(), autoResumeSessions(), bootstrap(), clamp(), cleanupRendererSession(), doCloseSession(), doSpawn(), doSpawnShell() (+160 more)

### Community 2 - "Community 2"
Cohesion: 0.01
Nodes (86): resolveEnvWithMode(), setupConfigHandlers(), setupDraftHandlers(), setupEditorHandlers(), registerIPCHandlers(), setupAppHandlers(), setupConfigHandlers(), setupGamepadHandlers() (+78 more)

### Community 3 - "Community 3"
Cohesion: 0.03
Nodes (94): handlePlanDeleteConfirmButton(), hidePlanDeleteConfirm(), showPlanDeleteConfirm(), ensureOverlay(), hidePlanHelpModal(), isPlanHelpVisible(), showPlanHelpModal(), applySequenceBandLayout() (+86 more)
...
Cohesion: 0.05
Nodes (56): createBrowseButton(), showFormModal(), applyWorkingDirectories(), parseSeqItems(), refreshSequencesCache(), renderSequenceGroups(), showAddSequenceGroupForm(), showEditSequenceGroupForm() (+48 more)
...
Cohesion: 0.04
Nodes (30): useEscProtection(), hideEditorPopup(), asElement(), getActiveInputContext(), getEditableOwner(), getTerminalOwner(), isEditableElement(), isEditableElementInContainer() (+22 more)

### Community 10 - "Community 10"
Cohesion: 0.06
Nodes (23): actionToPtyData(), comboToPtySequence(), executeSequenceString(), appendText(), canonicalizeKey(), canonicalizeModifier(), formatAction(), formatSequencePreview() (+15 more)
...
Cohesion: 0.2
Nodes (3): HelmTelegramService, validateMobileFriendlyTelegramText(), decodeBase64Strict()
...
- **Thin community `Community 37`** (15 nodes): `pinned-dashboard.js`, `groupByDirectory()`, `PinnedDashboard`, `.appendSessionGroups()`, `.buildDashboardKeyboard()`, `.buildDashboardText()`, `.constructor()`, `.createOrUpdate()`, `.dispose()`, `.handleEditError()`, `.pinMessage()`, `.setInstanceName()`, `.start()`, `.stop()`, `.update()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 40`** (14 nodes): `HelmContextService`, `.appendContext()`, `.bindContext()`, `.constructor()`, `.createContext()`, `.deleteContext()`, `.getContext()`, `.listContexts()`, `.listPlanContexts()`, `.requireWorkingDirectory()`, `.resolvePlanRef()`, `.setContextPosition()`, `.unbindContext()`, `.updateContext()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 61`** (8 nodes): `spawner.js`, `ProcessSpawner`, `.getAllProcesses()`, `.getProcess()`, `.getProcessesByCliType()`, `.kill()`, `.killAll()`, `.spawn()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 62`** (7 nodes): `HelmPlanAttachmentService`, `.addPlanAttachment()`, `.constructor()`, `.deletePlanAttachment()`, `.getPlanAttachment()`, `.listPlanAttachments()`, `.resolvePlanRef()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 100`** (2 nodes): `Haptic Feedback Setting`, `Notifications Setting`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 139`** (1 nodes): `Modal Guard Regression Fixes`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 140`** (1 nodes): `Persisted Sessions`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 141`** (1 nodes): `File Structure Module Map`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `TelegramBotCore` connect `Community 21` to `Community 2`?**
  _High betweenness centrality (0.017) - this node is a cross-community bridge._
- **Why does `StateDetector` connect `Community 27` to `Community 2`?**
