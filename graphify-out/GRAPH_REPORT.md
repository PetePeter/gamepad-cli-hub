...
## Corpus Check
- 384 files · ~3,150,857 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3268 nodes · 6456 edges · 80 communities detected
...
Cohesion: 0.01
Nodes (199): autoResumeSessions(), bootstrap(), clamp(), cleanupRendererSession(), doCloseSession(), doSpawn(), doSpawnShell(), findProjectForPath() (+191 more)
...
Cohesion: 0.05
Nodes (62): createSortControl(), sortBindingEntries(), createBrowseButton(), getCliDisplayName(), getCliIcon(), showFormModal(), createSpawnButton(), applyWorkingDirectories() (+54 more)
...
Cohesion: 0.06
Nodes (24): actionToPtyData(), comboToPtySequence(), executeSequenceString(), appendText(), canonicalizeKey(), canonicalizeModifier(), formatAction(), formatSequencePreview() (+16 more)
...
- **Thin community `Community 25`** (22 nodes): `BindingsTab.vue`, `PlansGrid.vue`, `ProfilesTab.vue`, `SessionCard.vue`, `SessionGroup.vue`, `SessionList.vue`, `SettingsPanel.vue`, `SortBar.vue`, `SpawnGrid.vue`, `StatusStrip.vue`, `TelegramTab.vue`, `ToolsTab.vue`, `colClass()`, `onCardClick()`, `onRenameKeydown()`, `selectState()`, `handleButton()`, `navigateTab()`, `makeCardProps()`, `makeGroupProps()`, `makeSessionListProps()`, `sidebar.test.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 26`** (22 nodes): `keyboard.js`, `keyboard-handlers.js`, `keyboard.js`, `setupKeyboardHandlers()`, `KeyboardSimulator`, `.comboDown()`, `.comboUp()`, `.constructor()`, `.getKeyDelay()`, `.keyDown()`, `.keyTap()`, `.keyUp()`, `.longPress()`, `.normalizeKey()`, `.sendKey()`, `.sendKeyCombo()`, `.sendKeys()`, `.setKeyDelay()`, `.typeString()`, `keyboard-handlers.ts`, `keyboard.ts`, `keyboard.test.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `TerminalManager` connect `Community 14` to `Community 1`?**
