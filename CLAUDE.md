...
25. **Config boundary — repo ships defaults only, runtime lives in the per-user app-data dir** — Writable config, logs, and temp files are always resolved to `<appData>/Helm/{config,logs,tmp}` regardless of dev or packaged mode, where `<appData>` is the platform-appropriate base matching Electron's `app.getPath('appData')`: `%APPDATA%` on Windows, `~/Library/Application Support` on macOS, `$XDG_CONFIG_HOME` (or `~/.config`) on Linux (`defaultAppDataBase()` in `src/utils/app-paths.ts`). The repo `config/` directory was removed from git tracking; only seed stubs remain in `src/config/` (settings.yaml, sessions.yaml, drafts.yaml, profiles/default.yaml, scheduled-tasks.yaml). On first launch, `seedConfigIfNeeded` copies these seeds to the user data dir if the target does not yet exist. `migrateLegacyUserDataIfNeeded` relocates data from the old bare `$HOME/Helm` fallback (written by pre-platform-aware builds) into the correct dir. This prevents dev builds from accidentally writing to the repo working tree and ensures packaged and dev behavior are identical.
...
28. **Recycle Bin (closed recoverable sessions)** — When a session that carried a `cliSessionName` (resume UUID) is closed, it is snapshotted to a rolling 30-day bin instead of vanishing entirely. Managed by `RecycleBinManager` (`src/session/recycle-bin-manager.ts`, EventEmitter, mirrors the ScheduledTaskHistoryManager rolling-window pattern — prune on append + defensive re-filter on load, injectable clock), persisted to `%APPDATA%/Helm/config/recycle-bin.yaml`. The `session:removed` listener routes through `recordRemovedSession()`, which keeps the existing auto-bookmark side effect and adds the bin entry under the same condition (has `cliSessionName` + `workingDir`); ephemeral sessions are never recorded. Each entry also captures the session's `projectId`/`projectName` at close time (resolved via `ProjectStore.findByPath`/`getById` in `handlers.ts` — never `resolveForPath`, so a close can't spawn a phantom project) plus its `runtimeGroupId`/`runtimeGroupName`, so the bin can render a Project ▸ Group ▸ Folder hierarchy without re-resolving. Restore reuses the normal spawn-with-resume flow: `recycleBin:restore` returns the entry and removes it, then the renderer calls `doSpawn(cliType, workingDir, _, cliSessionName)` — identical to startup resume. IPC: 4 channels (`recycleBin:list/restore/forget/empty`) + `recycle-bin:changed` event (new `recycleBin` preload domain). UI: a 🗑️ Recycle Bin button with a live badge count sits in the Project Planner section (bottom-left), opening `RecycleBinModal.vue` — a **nested, collapsible, searchable tree** (`Project ▸ Runtime Group ▸ Folder ▸ sessions`, ungrouped folders sit at project level with no group wrapper) built by the pure, unit-tested `renderer/recycle-bin-tree.ts` (`buildRecycleTree(entries, resolveProject, query)`; the modal's `resolveProject` prefers the entry's stored project, falling back to a `state.projects` working-dir lookup for legacy entries, else a synthetic `(no project)` node). Every level folds independently with a live descendant count; a search box filters across name/CLI/path and auto-hides emptied levels (header count switches to "N shown"). Rows show name + plain-text CLI badge, full working-dir path (tooltip + ellipsis), an optional runtime-group chip, and `closed Xm ago · yyyy/mm/dd HH:mm`. Per-folder **↺ Restore all / 🗑 Forget all** loop the single-entry IPC. Expiry/retention is enforced in the manager but intentionally NOT surfaced in the UI (no countdown, no colour bars). Reactive state lives in the `useRecycleBin` composable (module-singleton refs shared by badge + modal).
...
31. **Artifact Manager + Viewer (ephemeral per-session reports)** — A per-session, **ephemeral** store of renderable outputs an AI produces for the user to *read* (explanations, reports, analyses, results — NOT code), shown in a dedicated in-app panel instead of the user opening a file. Managed by `ArtifactManager` (`src/session/artifact-manager.ts`, EventEmitter, injected `persist` + injectable clock, mirrors DraftManager) — `create` (always a new uuid; duplicate titles allowed) `/update(id)` (append version) `/reveal(id)` (bring forward, no mutation) `/get/getForSession/count/delete/deleteAllForSession/clearSession/exportAll/importAll`; emits `artifact:changed` (persist sink) and `artifact:reveal` (create/update/reveal). Persisted to `%APPDATA%/Helm/config/artifacts.yaml` (global, keyed by session). **Versioned**: each artifact holds an ordered `versions[]` stack; the AI writes only the head, the user reads any prior version. **Lifecycle tied to the recycle bin**: on close the `session:removed` listener keeps artifacts when the session is recoverable (goes to the bin) and clears them only for a non-recoverable/ephemeral close. Recycle-bin **restore reuses the session's original id** (like startup auto-resume), so its preserved artifacts come straight back with no re-keying; only **Forget/Empty** clears a binned entry's artifacts. On startup `pruneOrphanArtifacts` reclaims artifacts whose session is neither live nor in the bin (e.g. crash orphans, or a bin entry that expired past the 30-day window). Content is **markdown or HTML**, always rendered through `renderArtifact()` (`renderer/artifacts/render-artifact.ts`) → synchronous `marked` → **`DOMPurify.sanitize`** before `v-html` (AI content is untrusted). **Local image support**: the AI writes plain `![alt](C:\path\to\file.png)` or `<img src="…">`; `resolveImageSrc()` rewrites local absolute paths and `file:` URIs to `helm-img://f/?p=<encodeURIComponent(absPath)>`, which the privileged `helm-img://` Electron protocol handler (`src/electron/helm-img-protocol.ts`) serves as raw bytes from the filesystem — `webSecurity` stays ON. Allowed sources: local absolute paths (Windows `C:\`, UNC `\\`, POSIX `/`) and `data:image/*` (non-SVG). https/http and SVG are dropped (https by design decision; SVG because it can embed scripts — serving it via a privileged scheme with the preload attached would be a security risk). **Text is selectable and copyable**: pressing Ctrl+C or Ctrl+X with a text selection inside the artifact document (`.ap-doc`) performs a native browser copy instead of sending SIGINT to the PTY — controlled by `shouldAllowNativeCopy()` in `renderer/paste-handler.ts`. No selection = existing PTY behaviour unchanged. **MCP surface (7 tools, session from auth context):** `artifact_create/update/show/delete/delete_all/list/get` — id-based tools additionally verify caller ownership (`requireOwnedArtifact`, uniform `Artifact not found` to avoid existence leaks); `session_info` advertises the viewer. IPC: `artifact:list/get/delete/deleteAll/reveal/export` (export = native save dialog) + `artifact:changed`/`artifact:reveal` events forwarded to the main and the session's own window. UI: `ArtifactViewer.vue` — a resizable right-hand **master/detail** panel (index rail with search/sort/Today-Earlier groups/unread dots/collapse; detail pane with version ‹›+dropdown, older-version banner, Export/Delete/Clear-all). Reactive state in the `useArtifactViewer` composable (module-singleton). Show/hide via a `📄 Artifacts` toolbar toggle (badge), `✕`, or `Ctrl+Shift+A`; collapsed = terminal full width + a pulsing right edge tab. **Snap-out follow**: the panel is bound to its session, so `SnapOutWindow` renders its own `ArtifactViewer` and the panel travels with a snapped-out terminal (never shown in two windows at once). See [docs/artifact-viewer.md](docs/artifact-viewer.md).
    ...
    graph LR
        ...
        VIEW --> RENDER["renderArtifact<br/>resolveImageSrc + marked + DOMPurify"]
        RENDER -->|"helm-img://f/?p=path"| PROTO["helm-img:// protocol<br/>(main process)"]
        PROTO -->|"readFile(absPath)"| FS[("filesystem")]
        VIEW -->|list/delete/export| AM
        SR["session:removed"] -->|"keep if recoverable<br/>else clearSession"| AM
        BIN["Recycle bin"] -->|"forget/empty → clear<br/>restore → reuse id"| AM
    ```

32. **Cross-Machine Helm Fleet (MCP peer proxy)** — (formerly "Federation"; the pre-rename `federation:` settings key is still read as a migration input.) One Helm invokes another Helm's MCP tools over the same LAN, so a local AI can drive a remote machine's sessions/plans/etc. through the remote's *native* tool names — no catalogue explosion. A SEPARATE TLS-WebSocket `RemoteLink` (default `:47474`, wholly distinct from the localhost `127.0.0.1:47373` MCP server, which is never touched) carries JSON-RPC 2.0. Transport is owned by `PeerLinkManager` (`src/mcp/peer/peer-link-manager.ts`, 1 `RemoteLinkServer` + N `RemoteLinkClient`, per-pair dedup) over `PeerLink` (JSON-RPC mux). Auth = a mutual **PSK handshake** (`remote-link-handshake.ts`, HMAC channel-bound to the TLS session via RFC-5705 exporter, hard-fail if unavailable) + **TOFU cert pinning** (`PinnedCertStore`, record-on-first-pair, hard-reject on change). Peers **pair by comparing a 6-digit SAS code** (`PeerPairing`/`pairing-crypto`: X25519 ECDH, commit-then-reveal, the code is a KDF OUTPUT the user compares — no offline verifier; `PairingCoordinator` enforces one-at-a-time + rate caps, inbound included) after mDNS `PeerDiscovery` (`_helm._tcp`). Pairing frames travel over a **`PairingSocket`** (`src/mcp/peer/pairing-socket.ts`) — a TLS-WS connection on the SAME `:47474` listener at path **`/pair`**, which skips the PSK handshake because the PSK is what pairing derives. Authenticity comes from each side folding the cert fingerprint observed on its OWN socket into the SAS transcript, so a MITM shows up as diverging codes; a `/pair` socket never becomes a `PeerLink` and can invoke no tool. Since mDNS does not cross subnets or survive Wi-Fi client isolation, **pair-by-address** (`peer:startPairingByAddress`) is a first-class path: the peer's machineId is learned from its own `hello` and bound into the SAS (a known machineId is never overwritten). `FleetController.status()` (`config:getFleetStatus`) reports `running`/`error`/reachable addresses so a dead stack is visible in the UI instead of looking like an empty LAN, and `reachableAddresses()` expands a `0.0.0.0` bind into the IPv4 addresses the user must type on the other machine. Every inbound call passes the `InboundCallGate` (`src/mcp/peer/inbound-call-gate.ts`): per-peer **allow-list** (`PeerConfigManager.isToolAllowed`, deny-by-default) → **hard-deny** set (`restart_helm`/`session_close`/`session_group_close`, never proxyable even under `*`) → per-peer **rate-limit** (token bucket) → dispatch under a synthetic **proxy identity** `peer:<id>` (`proxy-identity.ts`, no impersonation; caller-identity args stripped) → **7-day audit** (`PeerAuditLog`, arg KEY NAMES only, never values). The gate reuses the existing `callMcpTool` dispatcher **UNTOUCHED**. Local surface = 3 MCP tools (`peer_list`/`peer_tools`/`peer_call`) behind `HelmPeerService` (`src/mcp/services/helm-peer-service.ts`). Config in `%APPDATA%/Helm/config`: `peers.yaml` (registry+allow-list, no secrets), `peer-secrets.yaml` (PSK bytes), `peer-pins.yaml` (cert pins), `machine-identity.yaml`, `self-signed-cert.yaml`, `peer-audit.yaml`. UI: a **Peers** settings tab (`PeersTab.vue` + `FleetConfigPanel.vue` + `PeerPairingDialog.vue` + `PeerAuditModal.vue`, `usePeers`) — discovery shows a tri-state (off / not running / scanning / N found) plus a pair-by-address box; `PeerPairingDialog` is mounted app-wide and `usePeers.ensureSubscribed()` runs from `MainWindowApp` so an INBOUND pairing raises its dialog even if Settings was never opened. **OFF by default** — no `:47474` listener bound and no manager constructed until enabled (`fleet-startup.ts`). Deny messages are uniform ("Tool not permitted") so a peer cannot probe which tools exist. See [docs/fleet.md](docs/fleet.md).
...
| [docs/fleet.md](docs/fleet.md) | Cross-Machine Fleet — peer MCP proxy over same-LAN TLS-WS (:47474), PSK handshake + TOFU cert pinning, mDNS SAS pair-by-code, InboundCallGate (allow-list + proxy identity + rate-limit + 7-day audit + hard-deny), peer_list/peer_tools/peer_call, Peers settings tab, OFF by default |
