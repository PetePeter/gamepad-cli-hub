...
export function setupToolsHandlers(configLoader: ConfigLoader): void {
  ...
  ipcMain.handle('tools:addCliType', (
    ...
    options?: { env?: EnvVarEntry[]; handoffCommand?: string; renameCommand?: string; spawnCommand?: string; resumeCommand?: string; continueCommand?: string; helmInitialPrompt?: boolean; helmPreambleForInterSession?: boolean; largeTextAsTempFile?: boolean; pasteMode?: 'pty' | 'ptyindividual' | 'sendkeys' | 'sendkeysindividual' | 'clippaste'; submitSuffix?: string },
  ) => {
    try {
