...
/**
 * Execute a prompt/sequence string through a common PTY-oriented pipeline.
 *
 * Semantics:
 * - Text chunks are buffered and delivered through the configured text provider.
 * - {Enter} appends a CR into the current text buffer.
 * - {Send} flushes the current buffer, then writes a separate CR submit byte.
 * - Keys/combos flush first, then write their PTY escape/control sequence.
 * - Wait flushes first, then delays.
 */
