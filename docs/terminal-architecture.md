...
| TerminalView | `renderer/terminal/terminal-view.ts` | xterm.js wrapper with fit/search addons, OSC title change callback. Optional `onScrollInput` callback for scroll-specific PTY writes. Unified `scroll(direction, lines)` method handles both buffer modes + virtual alt screen: true normal buffer → `scrollLines()` directly (bypasses SmoothScrollableElement — xterm.js #5620); alternate buffer OR virtual alt screen → PageUp/PageDown escape sequences to PTY via `onScrollInput` (falls back to `onData`). Capture-phase wheel handler and gamepad scroll both route through `scroll()`. Virtual alt screen tracking: `updateVirtualAltScreen(rawData)` detects alt screen enter/exit from raw PTY data (before pty-filter strips them) — lets scroll input route to the CLI during TUI mode instead of scrolling the viewport |
| TerminalManager | `renderer/terminal/terminal-manager.ts` | Multi-terminal switching, lifecycle. `deselect()` pauses keyboard relay without destroying terminal. Accepts `contextText` forwarded to main process via `ptySpawn()`. `adoptTerminal()` creates a TerminalView for externally-spawned PTY sessions without calling `pty:spawn`. Owns `PtyOutputBuffer` for preview data. `setOnTitleChange()` routes terminal title events to renderer state. `writeToTerminal()` calls `updateVirtualAltScreen(rawData)` on the view (when `stripAltScreen` is active) then runs PTY output through `applyPtyFilters(data, { stripAltScreen })` before writing to xterm.js |
| PtyFilter | `renderer/terminal/pty-filter.ts` | Strips mouse-tracking and alternate-screen ANSI escape sequences from PTY output so native text selection works. Unified `applyPtyFilters(data, opts?)` entry point — always strips mouse tracking (DEC modes 1000–1007, 1015–1016), conditionally strips alt screen modes (47/1047/1048/1049) and ED 3 (`\x1b[3J`). ED 2 (`\x1b[2J`) intentionally preserved. `stripMouseTracking()` and `stripAltScreen()` are convenience wrappers. Fast-path skips regex when no escape sequences present. TerminalView tracks alt screen enter/exit BEFORE stripping via `updateVirtualAltScreen()` |
