...
- **Hard-deny list.** `restart_helm`, `session_group_close` are NEVER remotely invocable, even under a wildcard `*` allow-list (`HARD_DENY_TOOLS`). `session_close` is **ownership-gated**: a peer may close only sessions it created via `session_create` (tracked by `createdByPeerId`). Attempts to close sessions owned by another peer or created locally are denied with the same uniform message. Deny messages are **uniform** ("Tool not permitted") so a peer cannot probe which tools or sessions exist.
